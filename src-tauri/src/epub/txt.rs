...
fn split_chapters(
    ...
    //     2. 限长 —— 关键词后缀 ≤12 字（真标题都很短：番外篇 3 字、完本感言 4 字）；
    ...
    let title_regex = Regex::new(
        r"^(?:(?:第\s*[0-9一二三四五六七八九十百千万零〇两]+\s*[卷部篇章]|[0-9一二三四五六七八九十百千万零〇两]+[卷部篇章]).*|(?:番外|楔子|引子|序章|序言|序幕|序|尾声|终章|后记|作者的话|(?:完结|完本|新书|上架|正文|作者)?感言)[^。！？；，、]{0,12})$",
    )
