...
fn split_chapters(
    ...
    //   —— 以上两条是强特征，标题后可跟任意文字
    //   弱关键词分支（这些词在正文里也会出现，所以额外设限）：
    //     番外…                                    → 番外 / 番外篇 / 番外一 婚后生活
    //     (完结|完本|新书|上架|正文|作者)?感言…   → 完本感言 / 完结感言 / 感言
    //     楔子/引子/序章/序言/序幕/序/尾声/终章/后记/作者的话 + 短后缀
    //   弱分支的三重保护（实测语料里正文确实会出现「…发表获胜感言的时候…」）：
    //     1. 顶格 —— 正文段首缩进（全角空格）天然被排除；
    //     2. 限长 —— 关键词后缀 ≤20 字。真标题可以比想象的:`番外： 七夕咏流传
    //        （七夕快乐）` 后缀就有 13 字(曾被 12 字上限漏掉);再长的基本是
    //        没标点的正文碎片了;
    //     3. 排除句读 —— 后缀不得含 。！？；，、 （正文句子几乎必带其一，
    //        这条才是主力：放宽长度后仍靠它挡住正文）。
    ...
    let title_regex = Regex::new(
        r"^(?:(?:第\s*[0-9一二三四五六七八九十百千万零〇两]+\s*[卷部篇章]|[0-9一二三四五六七八九十百千万零〇两]+[卷部篇章]).*|(?:番外|楔子|引子|序章|序言|序幕|序|尾声|终章|后记|作者的话|(?:完结|完本|新书|上架|正文|作者)?感言)[^。！？；，、]{0,20})$",
    )
