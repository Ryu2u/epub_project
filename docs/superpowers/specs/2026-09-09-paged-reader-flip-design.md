...
**首测反馈修复(2026-09-09 第二轮)**:

6. **「每章只有一页」**:后端 `text_to_xhtml` 生成的章节以 `<body>\n` 开头,前端解析后首个子节点是空白文本节点——它没有行盒,`boxOf(0,1)` 返回 null,误触发「整章单页」兜底。修复:每页先**扫描跳过无可测几何的位置**(空白划归上一页末尾),二分中 null 区间视为装得下;回归测试用「空白原子无行盒」的假几何钉死该形态;
7. **舞台卡片样式反馈**:去掉 `.paged-stage` 的圆角+阴影(用户不需要书页卡片感);测量容器从 `visibility: hidden` 改为仅离屏定位(双保险保证 getClientRects 有真实行盒)。

**第三轮反馈(2026-09-09):移除仿真翻页,改为平滑平移**

8. 用户明确不需要仿真卷页,要求「左右平移」的平滑翻页 —— 删除 CurlFlip/snapshot/curlGeometry 全部实现与预热降级机器(-5 文件),翻页效果收敛为三种:**平移**(默认;双页刚性平移:当前页滑出的同时新页从同侧滑入,轮播式)、**覆盖**(新页从侧面滑入盖住当前页,带投影)、**无动画**(瞬翻)。`FlipStyle` 类型同步收敛为 `'cover' | 'slide' | 'none'`,存量 'curl' 偏好回落默认。§6 的仿真设计保留作为历史记录,不再对应任何代码。

**第四轮反馈(2026-09-09):翻页完成瞬间的旧页闪现**
...
**第六轮反馈(2026-09-09):章节标题移入正文**

14. 顶部小字标题栏取消,章节标题改为 `<h3>` 注入正文首位参与排版(第 1 页页首即标题,跨章翻页时被揭示页也带下一章标题):`parseChapterSource(html, title)` 注入;若正文自带同名首个标题则统一为 h3 不重复注入(与滚动模式的首个标题去重同源);标题纳入 `cacheKey`(标题单独编辑也会失效缓存);舞台高度不再预留标题栏(多出一行余量),章节进度并入页脚「第 x / N 章 · y / M 页」。
15. **翻页自动收起工具栏**:PagedReaderView 新增 `onPageTurn` 回调(手势/键盘/跨章直跳的翻页真正开始时触发),Reader 收到后 `setToolbarVisible(false)`;中央点击呼出/隐藏的既有交互不变。
