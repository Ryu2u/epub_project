...
- **🔎 全文搜索(逐次命中)** — SQLite FTS5 索引章节正文,结果**按「一次出现一条」**返回(全书命中总数与命中章节数分开统计,分页加载,不再一章一条);点击结果**跳到阅读器命中处并高亮**(滚动/分页两种模式都支持;定位用「章内第 N 次 + 命中前上下文」消歧,不受纯文本与渲染 DOM 空白差异影响);查询词少于 2 个字符时返回空
