# 📚 EPUB Reader + 书籍库(桌面客户端 / Web)

一个基于 Web 的 EPUB 阅读器与个人书籍库管理系统,默认以 **Tauri 2 桌面客户端**运行(Rust 业务库内置于 `src-tauri/`,原 axum HTTP 层已替换为 `#[tauri::command]`),前端使用 **React + TypeScript**,同一套界面也能在浏览器里跑,追求 iOS Books 般的阅读体验。支持 EPUB 和 TXT 两种来源格式。
...
- **📱 移动端主页 + 书库** — 参考阅读 App 主页:之前读过 / 阅读目标(半圆进度环 + 周历 + 连续阅读 + 继续阅读)/ 今年读过的图书;书库为封面网格(进度百分比 + 新增徽标 + 排序 + 卡片菜单;封面参考 iOS 图书:真实封面原图直出、悬停轻抬 + 投影随主题加深、删除封面二次确认);浅色蓝调 + 深色金调**可切换主题**;阅读时长与连续天数自动统计并持久化(localStorage)
- **📊 删除/导入/导出实时进度** — 三者均走异步任务 + 进度轮询(原 SSE 流):删除大书按批删章节(进度条 + 阶段消息,告别无反馈转圈);导入全程字节进度 + 解析/入库阶段进度(TXT 解析按行增量回报);导出阶段进度 + 完成下载
- **📚 多格式支持**
  - **EPUB 3 解析** — 完整的元数据提取(标题、作者、封面、目录等),EPUB 2 NCX 目录回退,非严格 XHTML 容错
  - **TXT 自动切章** — 整本 TXT 小说按章节标题自动切分,入库即可阅读,**编码自动检测**(UTF-8 / GBK / GB18030 / Big5 / UTF-16,统一转为 UTF-8 入库)。识别四类标题:
    - 顶格 `第X卷/部/篇/章`(X 支持阿拉伯与中文数词,如 `第一章`/`第12卷`/`第 3 章`),标题后可跟任意文字;
    - **允许前导空白**的 `第X卷/部/篇/章`(空格 / 全角空格 / Tab 均可;前面出现任何其它字符即按正文处理),但要求整行较短且不含句读;
    - 顶格的弱关键词标题:`番外`(番外 / 番外篇 / 番外一 婚后生活)、`楔子 / 引子 / 序章 / 序言 / 序 / 尾声 / 终章 / 后记 / 作者的话`、`(完结|完本|新书|上架|正文|作者)?感言`(如 `完本感言`、`番外： 七夕咏流传（七夕快乐）`);这类词在正文里很常见,所以要求顶格 + 后缀 ≤20 字 + 不含句读 `。！？；，、`;
    - 顶格的无「第」形式 `X卷/部/篇/章`(如 `1章`、`一百章`)。

    首个标题前的版权页等内容自动丢弃。规则细节与取舍见 `src-tauri/src/epub/txt.rs` 的注释。
- **📑 章节编辑器** — CodeMirror 源码 + 实时预览,支持在线编辑章节标题与 HTML 内容;编辑后的 HTML 落盘到存储目录
- **🖊️ 在线阅读器** — 章节级阅读,阅读进度自动保存与恢复(localStorage),内置目录面板
- **📖 分页阅读模式(平滑翻页)** — 与滚动阅读并存的逐页翻页模式:layout-then-slice 分页引擎(离屏测量容器 + `Range.getClientRects` 行盒二分切页,「测量=渲染」同一宽度来源)、锚点进度持久化(字号/行距/窗口变化保位重排,翻回上一章落回离开时的页)、三种翻页效果(**平移**(左右轮播式滑动,默认)/覆盖(新页滑入盖住当前页)/无动画)、键盘翻页(←/→/PgUp/PgDn/Space/Home/End)、邻章预取跨章零等待;分页与滚动**位置互通**(分页模式顶栏进度实时更新,切换模式按当前位置对齐,不会掉回章首)。设置面板「阅读模式」切换,默认仍为滚动。设计文档见 [2026-09-09](docs/superpowers/specs/2026-09-09-paged-reader-flip-design.md)
...
- **📤 导出（EPUB / TXT）** — EPUB:重新打包成标准 EPUB 3(导出 XHTML 严格符合 Sigil/EpubCheck,段首缩进 `text-indent:2em` 内置);TXT:标题顶格、正文段首空两格的纯文本,与 TXT 导入的切章格式互为镜像;**桌面端走原生「另存为」**——选格式后弹保存对话框,导出完成由后端直接把文件写入指定路径(字节不经过前端),浏览器端仍是下载;导出时会重写样式表、CSS `url()` 与章间内链,导出的包不再丢原书样式、脚注与章间链接可点
- **⚡ 虚拟化列表** — 章节列表与详情页目录使用 react-window 虚拟滚动,大书不卡顿
- **⚠️ 完善的错误处理** — DRM 检测、损坏文件识别、重复上传提示(按 SHA-256 去重)、编码错误提示;页面级**错误边界**(组件抛错不再整页白屏,可一键重试或回主页)
...
- **📖 分页阅读(分页模式)** — ✅ **已实现**(见上方功能特性)。设计文档:[2026-09-09 分页阅读设计](docs/superpowers/specs/2026-09-09-paged-reader-flip-design.md)。
  ...
  - 高于整页的插图已按页内容区高等比缩小(视口级 CSS 变量,测量容器与页面容器取值一致,「测量 = 渲染」不破)。
  - 待打磨(Phase 3):超长章节 idle 分页的更细粒度让出。
...
| 错误处理 | thiserror + 自定义 `EpubError`(业务层)/ `CmdError`(命令层) |
...
- **书库迁移/备份** — 书库页「迁移」按钮:整库导出为 `.epublib`(书目 + 章节 + 源文件 + 封面),拷到另一台电脑导入即可合并(同 id/SHA 自动跳过,FTS 索引自动重建);导入时校验归档条目路径(拒绝目录穿越)并限制解压总量,打包与写盘均为「临时文件 + rename」的原子写
...
> 开发服务器固定跑在 **http://localhost:15173**:端口特意避开 Windows 系统保留区间与临时端口池(1024-15000),
> `vite.config.ts` 的 `server.port` 与 `src-tauri/tauri.conf.json` 的 `devUrl` 必须保持一致。
>
