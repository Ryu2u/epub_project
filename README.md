...
- **📖 分页阅读模式(仿真翻页)** — 与滚动阅读并存的逐页翻页模式:layout-then-slice 分页引擎(离屏测量容器 + `Range.getClientRects` 行盒二分切页,「测量=渲染」同一宽度来源)、锚点进度持久化(字号/行距/窗口变化保位重排,翻回上一章落回离开时的页)、四种翻页效果(**仿真卷页**(Canvas 移植经典 PageWidget 贝塞尔几何,纸背反射/折痕阴影/快照失败自动降级)/覆盖/平移/无动画)、键盘翻页(←/→/PgUp/PgDn/Space/Home/End)、邻章预取跨章零等待。设置面板「阅读模式」切换,默认仍为滚动。设计文档见 [2026-09-09](docs/superpowers/specs/2026-09-09-paged-reader-flip-design.md)
